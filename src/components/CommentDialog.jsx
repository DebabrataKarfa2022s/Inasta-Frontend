...
//     try {
//       const response = await axios.post(`${SummaryApi.commentPost.url}/${selectedPost?._id}/comment`, { text }, {
//         headers: {
//           'Content-Type': 'application/json',
//         },
//         withCredentials: true
//       });
//       console.log("response from comment dialgo : ", response);
...
//     try {
//       const response = await axios.post(`${SummaryApi.followOrUnfollow.url}/${selectedPost?.author?._id}`, { withCredentials: true });
...
//           updatedFollowing = user.following.filter(id => id !== selectedPost?.author?._id);
//         } else {
...
//   return (
//     <Dialog open={open}>
//       <DialogContent onInteractOutside={() => setOpen(false)} className="max-w-xl md:max-w-3xl p-0 flex flex-col">
...
//                 </Link>
//                 <div>
//                   <Link className='font-semibold text-xs' to={`/${selectedPost?.author?._id}/profile`} >{selectedPost?.author?.username}</Link>
...
//                     <div onClick={followORUnfollow} className={`cursor-pointer w-full font-bold ${isFollowingAuthor ? 'text-[#ED4956] hover:text-[#97202a]' : 'text-[#139DF7] hover:text-[#1e699b]'} `}>
